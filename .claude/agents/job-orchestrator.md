...
1. **Pre-check**: controleer met **Glob** of er een CV beschikbaar is — eerst `cv/cv.pdf`, anders `cv/cv.md`. Lees dat bestand met **Read** (PDF wordt native ondersteund; gebruik de `pages`-parameter voor PDF's >10 pagina's). Als géén van beide aanwezig is, vraag de gebruiker om het CV te plaatsen vóór je begint.
2. **Parallelle dispatch**: roep de drie subagents gelijktijdig aan in één bericht met meerdere `Agent`-tool-calls:
   ...
   Elke subagent krijgt dezelfde basisopdracht: "zoek 5-10 relevante, actuele vacatures volgens jouw criteria; gebruik `cv/cv.pdf` (of terugval `cv/cv.md`) en `cv/wensen.md` als leidraad; schrijf output naar `vacatures/_raw/<drijfveer>-<timestamp>.json`".
...
5. **Scoring**: lees het CV zorgvuldig (`cv/cv.pdf` of `cv/cv.md`) en geef elke vacature een score **0-100** op basis van:
