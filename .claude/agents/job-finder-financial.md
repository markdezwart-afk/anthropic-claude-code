...
## Werkwijze
1. Lees eerst het CV van de gebruiker: probeer `cv/cv.pdf` (gebruik **Read** — die ondersteunt PDF's native; bij >10 pagina's gebruik je de `pages`-parameter). Als de PDF er niet is, val terug op `cv/cv.md`. Lees ook (indien aanwezig) `cv/wensen.md`.
